#!/usr/bin/env python3
"""
lstm_flowid_confusion.py — LSTM Inspector confusion matrix (v3 - IP haritalama ile)

Tuesday PCAP'ta saldırgan IP: 192.168.10.51
Tuesday CSV'de saldırgan IP:  172.16.0.1
Bu haritalama Flow ID üretiminde uygulanır.

Kullanım:
    python lstm_flowid_confusion.py \
        --alert-dir ~/bitirme/results/lstm \
        --csv-dir ~/bitirme/data/raw/cicids2017 \
        --output ~/bitirme/results/lstm/confusion_matrix_v3.txt
"""
...
# CIC-IDS2017'de Tuesday PCAP'taki 192.168.10.51, CSV'de 172.16.0.1 olarak kayıtlı
IP_MAP = {
...
def map_ip(ip):
    """PCAP IP'sini CSV IP'sine dönüştür."""
        ...
        with open(alert_file, 'r') as f:
            for line in f:
                ...
                try:
                    ...
                    # Orijinal IP'lerle Flow ID (PCAP IP'leriyle eşleşen CSV'ler için)
                    fid1 = f"{dst_ip}-{src_ip}-{dst_port}-{src_port}-{proto_num}"
                    fid2 = f"{src_ip}-{dst_ip}-{src_port}-{dst_port}-{proto_num}"
                    flow_ids.add(fid1)
                    flow_ids.add(fid2)

                    # Haritalanmış IP'lerle Flow ID (Tuesday gibi IP uyumsuzluğu olan günler için)
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("LSTM Alert → Flow ID (IP haritalama ile)")
    logging.info("=" * 60)
    ...
    result = f"""
{'=' * 60}
LSTM Inspector — CIC-IDS2017 Confusion Matrix (v3)
Yöntem: Flow ID + IP haritalama
Ayarlar: max_packets=10, threshold=0.55
